...
_Noreturn void StartDefaultTask(void const * arg)
{
    for(;;)
    {
        osDelay(1);
...
_Noreturn void read_uart(void const * arg){
...
_Noreturn void status_led(void const *args){
...
_Noreturn void adc_handler(void const *args){

    printf("Initialize AD7190");
    ad7190_init();
    ad7190_reset();
    HAL_Delay(1);
    ...
    if ((id &0x0f) != 0x04){
        printf("wrong id %x", id);
        osThreadSuspend(adc_handler_id);
    }
...
int main(void)
{
    ...
    osThreadDef(adc_handler, adc_handler, osPriorityNormal, 0, 256);
    adc_handler_id = osThreadCreate(osThread(adc_handler), NULL);
